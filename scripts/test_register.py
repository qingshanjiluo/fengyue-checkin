...
# -*- coding: utf-8 -*-
"""
网名库测试注册脚本 v2 — 精细滑块扫描 + 随机密码
用法: python test_register.py [数量] [代理]
"""
...
PROXY = sys.argv[2] if len(sys.argv) > 2 else os.environ.get('REG_PROXY', '')
NAMES_FILE = "网名库.txt"
...
async def main():
    ...
    log(f"网名库测试注册 v2: {COUNT}个 | 代理{'启用' if PROXY else '直连'}")
